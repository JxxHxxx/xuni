            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'EC2-ACCESS', keyFileVariable: 'PEM_KEY')]) {
                    dir('/var/lib/jenkins/workspace/xuni_ci_cd/build/libs') {
                        ...
                        sh "ssh -o StrictHostKeyChecking=no -i $PEM_KEY ubuntu@$API_REMOTE_SERVER_IP nohup java -jar /home/ubuntu/xuni-0.0.1-SNAPSHOT.jar --spring.config.name=application-dev > /dev/null 2>&1 &"
                    }
                }
            }
        }
